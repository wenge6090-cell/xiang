# 太极模型 (Taiji Model) — 三易全链路语义化实施计划

**版本：2.0** | **状态：设计完成，待路线B 跑通后启动**

> ⚠️ **路线C 实施计划** — 本文档是 [03-路线C-太极双LLM](./03-路线C-太极双LLM.md) 的详细工程实施步骤，属于远期架构愿景。  
>
> **在三易统一中的位置**：路线C 启动三引擎（归藏/连山/周易）从离散查表到全语义驱动的升级。  
> 当前已完成路线A（归藏引擎 — ✅ 已实现），路线B（控制原语微调 + 三引擎接线 — 📋 设计完成）为下一步重点。  
> 本计划待路线B 跑通完整周天后启动。
...
`CangVM` 新增字段:
- `semantic_origin: Option<SemanticOrigin>`  // SemanticOrigin = { text: String, embedding: Embedding }
...
**现状**: `LlmContext::generate_constrained_turn()` 在调用 `backend.generate()` 后**无条件**执行 `self.add_turn(user_input, &r.text)`。必须重构为条件路径。
...
**修改: `crates/xiang-llm/src/http_backend.rs`**
- `bias_to_logit_bias()` 返回 `force_stop=true` 时，需确保返回的 `GenerationResult.deviated = true`（新增字段）
...
**重要**: `generate_constrained_turn()` 的调用者（ExperimentRunner）需传递 origin_guidance 状态。签名字段不变，origin_guidance 通过 `self` 内部状态管理。
...
当前 `generate_constrained_turn()` 在调用 `backend.generate()` 后无条件执行 `self.add_turn(user_input, &r.text)`，即使 `stop_reason == Deviated`。这直接与"杀硬逻辑"要求冲突——物理丢弃要求 token 不进入 history。Phase 8 的"条件 add_turn"重构正是为了解决这个确切的代码路径缺陷。
